...
/* ---- shape drawers (sampled to particle targets) ----
   logo = the lab's Sprout-d: bowl ring + stem + two leaves   */
var SHAPES={
  logo:function(o,W,H,opt){
    var S=Math.min(W,H)*(opt.scale||.78),sc=S/120,ox=(W-S)/2,oy=(H-S)/2;
    var main=opt.main||'#1F5130',leafB=opt.accent||'#E0452B';
    o.save();o.translate(ox,oy);o.scale(sc,sc);
    o.strokeStyle=main;o.lineWidth=13;
    o.beginPath();o.arc(46,74,30,0,6.2832);o.stroke();
    o.fillStyle=main;o.fillRect(80,14,13,92);
    o.fill(new Path2D('M86 14 q2 -12 14 -14 q0 12 -14 14z'));
    o.fillStyle=leafB;o.fill(new Path2D('M100 14 q-2 -12 -14 -14 q0 12 14 14z'));
    o.restore();
  },
  check:function(o,W,H,opt){
    o.strokeStyle=opt.main||'#1F5130';o.lineWidth=Math.min(W,H)*.13;
...
function makeSys(canvas,mode,opt){
  ...
  function cols(){return opt.colors||['#1F5130','#E0452B','#F5C84C'];}
  ...
  function seedPollen(){
    P=[];for(var i=0;i<(opt.count||24);i++){
      P.push({bx:rnd(8,W-8),by:rnd(8,H-8),s:rnd(1.2,2.7),
        ...
        col:Math.random()<.75?(opt.glow||'#F5C84C'):(opt.glow2||'#7FB069')});
    }
  }
  /* formation spawner */
  function spawnForm(){
  ...
  function draw(){
    ...
    if(R.length){
      ...
      for(i=0;i<R.length;i++){
        ctx.globalAlpha=Math.max(0,R[i].al);ctx.strokeStyle=opt.ring||opt.main||'#1F5130';
...
function makeBurstLayer(canvas){
  ...
  var sys={canvas:canvas,active:true,
    ...
    fire:function(x,y,colors,n){
      if(reduced)return;
      resize();
      colors=colors||['#1F5130','#E0452B','#F5C84C'];
      n=n||14;
      for(var i=0;i<n;i++){
